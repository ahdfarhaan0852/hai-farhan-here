...
import { type ProjectData } from "../../../constants/mohitvirliData";
...
export const ProjectTile: React.FC<ProjectTileProps> = ({
  ...
  const hovered = activeId === index || desktopHovered;
  ...
  useEffect(() => {
    ...
    const [mesh, title, dateGroup, textBox, button] = children;
    ...
    hoverAnimRef.current
      ...
      .to((mesh as THREE.Mesh).material, { opacity: hovered ? 0.95 : 0.35 }, 0)
  ...
  return (
    <group
      ...
      onPointerOver={(e) => {
        e.stopPropagation();
        if (isProjectSectionActive) setDesktopHovered(true);
      }}
      onPointerOut={() => isProjectSectionActive && setDesktopHovered(false)}
    >
