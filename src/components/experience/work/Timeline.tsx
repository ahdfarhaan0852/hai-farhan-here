...
import { AHMAD_FARHAN_WORK_TIMELINE, type WorkTimelinePointData } from "../../../constants/mohitvirliData";

const reusableLeft = new THREE.Vector3(-0.3, 0, -0.1);
const reusableRight = new THREE.Vector3(0.3, 0, -0.1);

const TimelinePoint = ({ point, diff }: { point: WorkTimelinePointData; diff: number }) => {
...
export const Timeline: React.FC<{ progress?: number }> = ({ progress = 1 }) => {
  const { camera } = useThree();
  const isActive = usePortalStore((state) => state.activePortalId === "work");
  const timeline = useMemo(() => AHMAD_FARHAN_WORK_TIMELINE, []);
